...
def set_goal_orientation(delta, current_orientation, prev_orientation, orientation_limit=None, set_ori=None):
    """
    Calculates and returns the desired goal orientation, clipping the result accordingly to @orientation_limits.
    @delta and @current_orientation must be specified if a relative goal is requested, else @set_ori must be
    an orientation matrix specified to define a global orientation
    ...
    if euler[2] > 0:
        if euler[2] < 2.87:
            limited = True
            goal_orientation = prev_orientation
    else:
        if euler[2] > -2.87:
            limited = True
            goal_orientation = prev_orientation
    ...
    # prevent joint from hitting the table
    if euler[0] > -1.54:
        limited = True
        goal_orientation = prev_orientation
    elif euler[0] < -1.64:
    ...
    return goal_orientation, prev_orientation