...
class OperationalSpaceController(Controller):
    ...
    def __init__(
        ...
        **kwargs,  # does nothing; used so no error raised when dict is passed with extra terms used previously
        ...
        # output_max = (0.01, 0.01, 0.01, 0.05, 0.05, 0.05)
        # output_min = (-0.01, -0.01, -0.01, -0.05, -0.05, -0.05)

        # this !!!!!
        # output_max = (0.01, 0.01, 0.005, 0, 0, 0.05)
        # output_min = (-0.01, -0.01, -0.005, -0, -0, -0.05)

        # output_max = (0.035, 0.035, 0,          0, 0, 0.5)
        # output_min = (-0.035, -0.035, 0,        -0, -0, -0.5)

        output_max = (0.05, 0.05, 0,          0, 0, 0.5)
        output_min = (-0.05, -0.05, 0,        -0, -0, -0.5)
        ...
        # self.position_limits = np.array([[-np.inf, -np.inf, 0.85529496],
        #                                  [np.inf, np.inf, 0.85529496]])
    
        self.position_limits = np.array([[-np.inf, -np.inf, 0.88],
                                         [np.inf, np.inf, 0.88]])

        # self.orientation_limits = np.array([[-np.inf, -np.inf, -np.inf],
        #                                     [np.inf, np.inf, np.inf]])
        
        # -1.6055421 -0.0054121, 2.87, -2.87
        # self.orientation_limits = np.array([[-1.605, -0.0054, 2.87],
        #                                     [-1.605, -0.0054, -2.87]])
        ...
        self.prev_orientation = self.ee_ori_mat
    ...
    def set_goal(self, action, set_pos=None, set_ori=None):
        ...
        if sum(bools) > 0.0 or set_ori is not None:
            ...
            # )
            self.goal_ori, self.prev_orientation = set_goal_orientation(
                scaled_delta[3:], self.ee_ori_mat, self.prev_orientation, orientation_limit=self.orientation_limits, set_ori=set_ori
            )
    ...
    def run_controller(self):
        ...
        # self.kp[0:3] = [300,300,150]
        # self.kd[0:3] = [150,150,300]
        ...
        if self.uncoupling:
            ...
            if self.policy_step:
                self.applied_force.append([decoupled_force])
                self.applied_torque.append([decoupled_torque])
                self.policy_step = False
        else:
